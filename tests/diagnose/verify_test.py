...
``test_reviewed_verifier_executes_against_rollout_claims``: it runs a real
``VerifierSpec`` through the real ``execute_verifier`` over evidence rendered
from a rollout. If the claim names or value shapes ever drift from
``analyze/outcomes.py``, that test fails and the whole capability number stops
being comparable to a historical one.
"""
...
from bandits.verify.models import CheckOperator, CheckSpec, VerifierSpec
...
def test_reviewed_verifier_executes_against_rollout_claims() -> None:
    """The whole point: one scorer, so history and rollout numbers compare."""
    ...
    spec = VerifierSpec(
        ...
        checks=(
            CheckSpec(
                check_id="check-1",
                claim="final_state_field:cancel_reservation.3RK2T9.status",
                operator=CheckOperator.EQUALS,
                expected="cancelled",
                supporting_evidence_ids=(),
                description="the named reservation is cancelled",
            ),
        ),
        unknown_when=(),
        blind_spots=(),
...
def test_a_wrong_terminal_value_fails_through_the_same_operator() -> None:
    ...
    assert execute_verifier(spec, evidence).score == 0.0


def test_a_path_the_rollout_never_observed_is_unknown_not_failure() -> None:
    """``execute_verifier``'s rule, preserved across the join."""
    evidence = claims_to_evidence(
    ...
    spec = VerifierSpec(
        ...
        checks=(
            CheckSpec(
                check_id="c",
                claim="final_state_field:cancel_reservation.X.status",
                operator=CheckOperator.EQUALS,
                expected="cancelled",
                supporting_evidence_ids=(),
                description="d",
    ...
    assert execute_verifier(spec, evidence).score is None
